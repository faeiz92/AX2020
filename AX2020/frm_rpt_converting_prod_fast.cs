...
namespace AX2020
{
	public partial class frm_rpt_converting_prod_fast : Form
	{
		...
		public frm_rpt_converting_prod_fast()
		{
			InitializeComponent();	
		...
		private DataSet GetData()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT PROD_MACHINE, SUM(PROD_QTYCTN) AS SUMQTYCTN, SUM(PROD_M2) AS SUMM2, SUM(PROD_TOTALROLL) AS SUMTOTALROLL FROM TBL_PROD_CONV_JO_QUICK_PROD_SUMMARY_TEMP GROUP BY PROD_MACHINE ORDER BY PROD_MACHINE", conn);
			try
			{
		...
		private DataSet GetData2()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT  SUM(PROD_TOTALROLL) as PROD_TOTALROLL, SUM(PROD_QTYCTN) AS PROD_QTYCTN, SUM(PROD_QTYM2) AS PROD_QTYM2, PROD_TYPE FROM TBL_PROD_CONV_JO_QUICK_PROD_TEMP group by PROD_TYPE order by PROD_TYPE", conn);
		...
		private DataSet GetData3()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT * FROM TBL_PROD_CONV_JO_QUICK_PROD_TEMP", conn);
			try
			{
		        SqlDataAdapter dataadapter = new SqlDataAdapter(cmd);
		        conn.Open();
...
//			else
//			{
				sqlConnParm("SP_PROD_CONV_JO_QUICK_PROD");
//				if(!Report())
//					return;
//			}			
		}
