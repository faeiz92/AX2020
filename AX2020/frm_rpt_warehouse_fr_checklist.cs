...
namespace AX2020
{
	...
	public partial class frm_rpt_warehouse_fr_checklist : Form
	{
		...
		public frm_rpt_warehouse_fr_checklist()
		{	
			InitializeComponent();
...
//			date_to = dtp_date_to.Value.Date.AddDays(1).AddSeconds(-1).ToString();
			...
			DataGrid("SELECT PROD_LINE, PROD_DOCNO, PROD_CUSTOMER, PROD_REPORTDATE, PROD_CODE, PROD_BRAND, PROD_COLOR, PROD_MICRON, PROD_WIDTH, PROD_LENGTH, PROD_QTYCTN, PROD_QTYROLL FROM TBL_PROD_WAREHOUSE_FR_RECEIVED where PROD_REPORTDATE>='"+ date_fr.ToString("yyyy-MM-dd") + "'and PROD_REPORTDATE<='"+ date_to.ToString("yyyy-MM-dd") + "' order by PROD_REPORTDATE DESC, PROD_LINE DESC", dt_grid);
...
//				}
		...
		void Txtbx_pack_noKeyUp(object sender, KeyEventArgs e)
		{
			...
			ds.DefaultView.RowFilter = "PROD_LINE LIKE '%" + txtbx_pack_no.Text + "%'";
