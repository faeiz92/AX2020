...
{
	/// <summary>
	/// Description of frm_po_local.
	/// </summary>
	public partial class frm_po_local : Form
	{
		...
		void datagrid()
		{
			string sql = "select * from  VIEW_AXERP_PO_ETD_LOCAL_WHSE order by deliverydate, purchname, itemid";
		...
		{
			string sql = "select * from  VIEW_AXERP_PO_ETD_LOCAL_WHSE  where deliverydate >= '" + date_from.Value.ToString("MM/dd/yyyy") + "' and deliverydate <= '" + date_from.Value.ToString("MM/dd/yyyy") +"'";
            SqlConnection connection = new SqlConnection(sqlconn3);
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, connection);
            DataTable ds = new DataTable();
            connection.Open();
		...
		{
			string sql = "select * from  VIEW_AXERP_PO_ETD_LOCAL_WHSE  where purchid like '%" + txtxbx_search_so.Text + "'" ;
            SqlConnection connection = new SqlConnection(sqlconn3);
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, connection);
            DataTable ds = new DataTable();
            connection.Open();
		...
		{
			string sql = "select * from  VIEW_AXERP_PO_ETD_LOCAL_WHSE  where name like '" + txtbx_name.Text + "%'" ;
            SqlConnection connection = new SqlConnection(sqlconn3);
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, connection);
            DataTable ds = new DataTable();
            connection.Open();
