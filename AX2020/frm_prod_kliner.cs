...
namespace AX2020
{
	...
	public partial class frm_prod_kliner : Form
	{
		...
		void Frm_prod_klinerLoad(object sender, EventArgs e)
		{
			ClearAllText(this);
			DataGrid();
...
//			}
		...
		void DataGrid()
		{
			...
			try 
			{
				cmd_SP.CommandText = @"SELECT PROD_DATE, PROD_SHIFT, PROD_DOCNO, PROD_STOCKCODE, PROD_STOCKDESC, PROD_QTY, PROD_UOM FROM TBL_PROD_KLINER_OUTPUT ORDER BY PROD_REPORTDATE DESC";
...
//            	ClearAllText(this);
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			if (LineNoGenerator())
			{
				if (sqlConnParmOutput("SP_PROD_KLINER_OUTPUT_ADD"))
				{
					for(i=0; i<rowCount; i++)
					{
						...
						if(!sqlConnParmConsume("SP_PROD_KLINER_CONSUME_ADD"))
							return;
					}
				}
...
//        rdlView.SourceFile = new Uri(baseUri,@"../report/prod_output_kliner.rdl");
//        
//        
//        rdlView.Report.DataSets["Data"].SetSource("select * from TBL_PROD_KLINER_CONSUME where PRODC_DOCNO = '" + txtbx_ref_no.Text + "'");
//        rdlView.Report.DataSets["Data2"].SetSource("select * from TBL_PROD_KLINER_OUTPUT where PRODC_DOCNO = '" + txtbx_ref_no.Text + "'");
//    
//        rdlView.Rebuild();
//
...
//		}
		...
		private bool LineNoGenerator()
		{
			...
			try 
			{
				cmdNextNumber.CommandText = "Select ProdKlinerNextNumber from TBL_ADMIN_NEXT_NUMBER";
				...
				rdNextNumber.Read();
				NextNo = Convert.ToInt32(rdNextNumber["ProdKlinerNextNumber"]);
			...
			try
			{
				cmdUpdateNextNumber.CommandText = "update TBL_ADMIN_NEXT_NUMBER set ProdKlinerNextNumber = ProdKlinerNextNumber + 1";
...
//				cmd_data_consume.Parameters["@SP_PRODC_USER"].Value = lbl_date.Text.ToUpper();
		...
		void Btn_delClick(object sender, EventArgs e)
		{
			...
			if (del == DialogResult.Yes)
            {
			    ...
			    {
					if (sqlConnParmOutput("SP_PROD_KLINER_OUTPUT_DEL"))
					{
						for(i=0; i< dt_grid_consume2.Rows.Count;i++)
						{
							...
							if(!sqlConnParmConsume("SP_PROD_KLINER_CONSUME_DEL"))
								return;
						}
...
//		void Btn_retrieveClick(object sender, EventArgs e)
		void Retrieve()
		{
			...
			try 
			{
				...
				cmd_SP2.CommandText = @"SELECT PRODC_STOCKCODE, PRODC_STOCKDESC, PRODC_QTY, PRODC_UOM FROM TBL_PROD_KLINER_CONSUME where SUBSTRING(PRODC_DOCNO,1,3) = @doc_no";
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			if (dt_grid_consume.Rows.Count < 10)
			{
				...
				using(frm_prod_kliner_popup frm_popup_kliner =  new frm_prod_kliner_popup())
				{
					frm_popup_kliner.ShowDialog();
					
					string prod_code_add = frm_prod_kliner_popup.prod_code;
					string prod_desc_add = frm_prod_kliner_popup.prod_desc;
					string prod_uom_add = frm_prod_kliner_popup.prod_uom;
