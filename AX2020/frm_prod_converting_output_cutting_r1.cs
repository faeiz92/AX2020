...
namespace AX2020
{
	...
	public partial class frm_prod_converting_output_cutting_r1 : Form
	{
		string init = "0";
		...
		string username, jr_lot_no, jr_ref_no, jr_color, jr_shipmark, jr_barcode;
		...
		public frm_prod_converting_output_cutting_r1()
		{
...
//            }
		...
		void Btn_checkClick(object sender, EventArgs e)
		{
			...
			{				
				...
				try 
				{
					cmd_Check_Duplicate.CommandText = @"select * from TBL_PROD_CONV_JO_CUTTING_SHIPMARK_R1 where PROD_SHIPMARK like @ship_mark + '%' and PROD_SHIPMARK <> '999'";// + "' and JODOCNO <> 'SMSO124608'";
				...
				catch (Exception ex)
				{
					...
					MessageBox.Show("Error - Converting Cutting Shipping Mark Duplicate " + ex.Message + ex.ToString());
					return;
				} 
				finally 
				{
				...
				try 
				{
					...
					cmd2.CommandText = "select * from TBL_PROD_CONV_JO_REWINDING_SHIPMARK_LR where PROD_SHIPMARKLR = @ship_mark + '%'";
					...
					if (rd2.HasRows)
					{
						while (rd2.Read())
						{								
				        	jr_shipmark = rd2["PROD_SHIPMARKLR"].ToString();
							jr_lot_no = rd2["PROD_JRLOTNO"].ToString();
							jr_ref_no = rd2["PROD_JRJONO"].ToString();
							txtbx_mic.Text = (Convert.ToInt32(rd2["PROD_JRMICRON"])).ToString();
							jr_color = rd2["PROD_JRCOLOR"].ToString();	
							jr_barcode = rd2["PROD_SHIPMARKLR"].ToString();
							
							txtbx_width.Text = (Convert.ToInt32(rd2["PROD_JRWIDTH"])).ToString();
			...
			else
			{
				...
				try 
				{
					//select from TBL_PROD_CONV_JO_REWINDING_SHIPMARK_LR where PROD_SHIPMARKLR = @ship_mark
					cmd_Check_Duplicate_JO.CommandText = @"select * from TBL_PROD_CONV_JO_CUTTING_SHIPMARK_R1 where PROD_SHIPMARK like @ship_mark + '%'";// + "' and JODOCNO <> 'SMSO124608'";
				...
				{
					//cmd.CommandText = "select twh, tstk, tlot_no, tref_no, twidth, tlength, tthickness, tcolor from stk_lot where tstk like 'WJ%' and twh = 'GF1' and tbal_qty > 1 '" + txtbx_ref_no.Text + "'";
					cmd.CommandText = "select select * from TBL_PROD_CONV_JO_REWINDING_SHIPMARK_LR where PROD_SHIPMARKLR = @ship_mark + '%'";
	...
	//			        	btn_cancel.Location = new Point(558, 912);	
							...
							jr_shipmark = rd["PROD_SHIPMARKLR"].ToString();
							jr_lot_no = rd["PROD_JRLOTNO"].ToString();
							jr_ref_no = rd["PROD_JRJONO"].ToString();
							txtbx_mic.Text = (Convert.ToInt32(rd["PROD_JRMICRON"])).ToString();
							jr_color = rd["PROD_JRCOLOR"].ToString();	
							jr_barcode = rd["PROD_SHIPMARKLR"].ToString();
							
							txtbx_width.Text = (Convert.ToInt32(rd["PROD_JRWIDTH"])).ToString();
							shipmark_ori_length = Convert.ToInt32(rd["PROD_JRLENGTH"]);
