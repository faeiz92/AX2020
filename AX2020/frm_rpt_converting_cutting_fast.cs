...
namespace AX2020
{
	...
	public partial class frm_rpt_converting_cutting_fast : Form
	{
		...
		public frm_rpt_converting_cutting_fast()
		{
			InitializeComponent();
		...
		private DataSet GetData()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT PROD_MACHINE, SUM(PROD_QTYCTN) AS SUMQTYCTN, SUM(PROD_M2) AS SUMM2, SUM(PROD_TOTALROLL) AS SUMTOTALROLL FROM TBL_PROD_CONV_JO_QUICK_CUTTING_SUMMARY_TEMP GROUP BY PROD_MACHINE ORDER BY PROD_MACHINE", conn);
			try
			{
		...
		private DataSet GetData2()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT * FROM TBL_PROD_CONV_JO_QUICK_CUTTING_TEMP", conn);
			try
			{
		        SqlDataAdapter dataadapter = new SqlDataAdapter(cmd);
		        conn.Open();
		...
		void Btn_searchClick(object sender, EventArgs e)
		{
			...
			date_to = dtp_date_to.Value.Date.AddDays(1).AddSeconds(-1);
				...
				sqlConnParm("SP_PROD_CONV_JO_QUICK_CUTTING");
//				if(!Report())
//					return;
		...
		void sqlConnParm(string cmd_update)
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_CUTTING_FAST.rdl";
					DataSet ds = new DataSet();
					DataSet ds2 = new DataSet();
					ds = GetData();
					ds2 = GetData2();
