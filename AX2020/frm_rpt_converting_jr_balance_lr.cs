...
namespace AX2020
{
	...
	public partial class frm_rpt_converting_jr_balance_lr : Form
	{
		...
		public frm_rpt_converting_jr_balance_lr()
		{
			InitializeComponent();
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT * FROM VIEW_PROD_CONV_JO_SLITTING_JR_BALANCE WHERE ((PROD_DATE BETWEEN @from_date AND @to_date) or (@from_date is null and @to_date is null)) and (PROD_JONO = @jo_no or @jo_no is null)", conn);
		...
		private DataSet GetData2()
		{
			...
			//SqlCommand cmd = new SqlCommand("SELECT PROD_MACHINE, SUM(PROD_QTYCTN) AS SUMQTYCTN, SUM(PROD_M2) AS SUMM2, SUM(PROD_TOTALROLL) AS SUMTOTALROLL FROM TBL_PROD_CONV_JO_QUICK_PACK_SUMMARY_TEMP WHERE PROD_DATE BETWEEN @from_date and @to_date GROUP BY PROD_MACHINE ORDER BY PROD_MACHINE", conn);
			SqlCommand cmd = new SqlCommand(@"SELECT * FROM VIEW_PROD_CONV_JO_SLITTING_JR_BALANCE WHERE ((PROD_DATE BETWEEN @from_date AND @to_date) or (@from_date is null and @to_date is null)) and (PROD_JONO = @jo_no or @jo_no is null) AND JOPRODREMARK5 = 'REWINDING + CUTTING'", conn);
