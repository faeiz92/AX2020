...
namespace AX2020
{
	public partial class frm_prod_glue : Form
	{
		...
		public frm_prod_glue()
		{
			InitializeComponent();
			this.ControlBox = false;
			PRODUCT_CODE_OPTION();
			//PRODUCT_CODE_DESC_OPTION();
			
			username = frm_menu_system.userName; 
			lbl_username.Text = "User : " + username;
			
			DropDownList("SELECT * FROM TBL_PROD_GLUE_MACHINE_NO_LIST order by MACHINE_NO", cbx_machine, "MACHINE_NO");	
			DropDownList("SELECT * FROM TBL_PROD_CONV_JO_OUTPUT_SHIFT", cbx_shift, "SHIFT");
			DropDownList("SELECT sysstaffname FROM TBL_ADMIN_USER_FACTORY where sysdept = 'GLUE'", cbx_operator, "sysstaffname");
			DropDownList("SELECT sysstaffname FROM TBL_ADMIN_USER_FACTORY where sysdept = 'GLUE'", cbx_helper, "sysstaffname");
...
//		}
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			if (dt_grid_consume.Rows.Count < 15)
			{
				    
				using(frm_prod_glue_popup frm_popup_glue =  new frm_prod_glue_popup())
				{
					frm_popup_glue.ShowDialog();
					
					string prod_code_add = frm_prod_glue_popup.prod_code;
					string prod_desc_add = frm_prod_glue_popup.prod_desc;
					string prod_uom_add = frm_prod_glue_popup.prod_uom;
