...
namespace AX2020
{
	...
	public partial class frm_rpt_stretch_film_output : Form
	{
		...
		public frm_rpt_stretch_film_output()
		{
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT PROD_DATE, PROD_SHIFT, PROD_MACHINE, PROD_DOCNO, 
											...
											FROM TBL_PROD_STRETCH_FILM_OUTPUT WHERE PROD_DATE BETWEEN @from_date AND @to_date", conn);
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath =  @"..\..\report\PROD_STRETCH_FILM_OUPUT.rdl";
					DataSet ds = new DataSet();
