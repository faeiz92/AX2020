...
namespace AX2020
{
	...
	public partial class frm_prod_converting_output_rewinding_r1 : Form
	{
		string init = "0";
		...
		string username, jr_lot_no, jr_ref_no, jr_color, jr_shipmark, jr_barcode, staff_id;
		...
		public frm_prod_converting_output_rewinding_r1()
		{
...
//		}
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			if(Double.Parse(txtbx_bal_m.Text) > (Double.Parse(txtbx_length.Text) - Double.Parse(txtbx_total_meter.Text)))
			{
			...
			if (LineNoGenerator())
			{
				...
				if (sqlConnParm("SP_PROD_CONV_JO_REWINDING_ADD") & sqlConnParmReject("SP_PROD_CONV_JO_REWINDING_REJECT_ADD") & sqlConnParmShipMark("SP_PROD_CONV_JO_REWINDING_SHIPMARK_ADD_R1"))
				{
...
//			}
		...
		bool sqlConnParm(string sqlStatement)
		{
			...
			try
			{
					...
					cmd_data.Parameters["@SP_PROD_QTYREJECT"].Value = Convert.ToDouble(txtbx_bal_m.Text);
		...
		bool sqlConnParmLR(string sqlStatement)
		{
			...
			try
			{
					...
					cmd_data.Parameters["@SP_PROD_SHIPMARKLR"].Value = DateTime.Now.Year.ToString().Substring(3,1) + DateTime.Now.ToString("MMdd") + jr_shipmark_short + prod_line + i.ToString("00");
		...
		void Btn_checkClick(object sender, EventArgs e)
		{
			...
			if(txtbx_shipping_mark.Text.ToUpper().Substring(0,3) == "BAL")
			{				
				...
				try 
				{
					cmd_Check_Duplicate.CommandText = @"select * from TBL_PROD_CONV_JO_REWINDING_SHIPMARK_R1 where substring(PROD_SHIPMARK,4,LEN(PROD_SHIPMARK)-4) like @ship_mark + '%' and substring(PROD_SHIPMARK,4,3) NOT LIKE '999%'";// + "' and JODOCNO <> 'SMSO124608'";
					cmd_Check_Duplicate.Parameters.AddWithValue("@ship_mark",  txtbx_shipping_mark.Text.Substring(4).ToUpper());
					
					cmd_Check_Duplicate.Connection = con_Check_Duplicate;
					con_Check_Duplicate.Open();
					SqlDataReader rd_Check_Duplicate = cmd_Check_Duplicate.ExecuteReader(System.Data.CommandBehavior.CloseConnection);
	
					if (rd_Check_Duplicate.HasRows)
					{
						if (rd_Check_Duplicate.Read())
						{
							//isDuplicate = true;
							shipmark_bal = Convert.ToInt32(rd_Check_Duplicate["PROD_BALANCE"]);
				...
				catch (Exception ex)
				{
					...
					MessageBox.Show("Error - Converting Rewinding Shipping Mark Duplicate " + ex.Message + ex.ToString());
					return;
				} 
				finally 
				{
				...
				cmd_Check_Duplicate.Dispose();
				con_Check_Duplicate = null;
				cmd_Check_Duplicate = null;
				
				
				//----------------missing shipmark--------------------------------------------------
				
				if(txtbx_shipping_mark.Text.ToUpper().Substring(0,7)=="BAL-999")
				{
					txtbx_mic.ReadOnly = false;
					txtbx_mic.Text = null;
					txtbx_length.ReadOnly = false;
					txtbx_length.Text = null;
					txtbx_width.ReadOnly = false;
					txtbx_width.Text = null;
					
					jr_barcode = txtbx_shipping_mark.Text.Substring(4, txtbx_shipping_mark.Text.Length-4).ToUpper();
					jr_shipmark = jr_barcode;
					jr_color = txtbx_color.Text;
					jr_lot_no = jr_barcode;
					jr_ref_no = txtbx_ref_no.Text.ToUpper();
					return;
					//MessageBox.Show("Incorrect Shipping Mark");
					//return;
				}
				...
				try 
				{
					...
					cmd2.CommandText = "select JRBarcode, TrxLotNo, TrxJoNo, TrxJoThickness, TrxJoWidth, TrxTotalLength, TrxJoColor, TrxShipMark, TrxExtraLength, TrxQCExtra from VIEW_CONVERTING_BARCODE where TrxShipMark like @ship_mark + '%' or JRBarcode = @ship_mark";
					cmd2.Parameters.AddWithValue("@ship_mark",  txtbx_shipping_mark.Text.Substring(4).ToUpper());
					...
					if (rd2.HasRows)
					{
						while (rd2.Read())
						{								
				        	jr_shipmark = rd2["TrxShipMark"].ToString();
							jr_lot_no = rd2["TrxLotNo"].ToString();
							jr_ref_no = rd2["TrxJoNo"].ToString();
							txtbx_mic.Text = (Convert.ToInt32(rd2["TrxJoThickness"])).ToString();
							jr_color = rd2["TrxJoColor"].ToString();	
							jr_barcode = rd2["JRBarcode"].ToString();
							
							txtbx_width.Text = (Convert.ToInt32(rd2["TrxJoWidth"])).ToString();
							//txtbx_length.Text = (Convert.ToInt32(rd["TrxTotalLength"])).ToString();
							//txtbx_length.Text = "0";
							//txtbx_extra.Text = (Convert.ToDouble(rd2["TrxExtraLength"]) + Convert.ToDouble(rd2["TrxQCExtra"])).ToString();
							txtbx_length.ReadOnly = false;
							txtbx_extra.ReadOnly = true;
			...
			else
			{
				...
				try 
				{
					cmd_Check_Duplicate_JO.CommandText = @"select * from TBL_PROD_CONV_JO_REWINDING_SHIPMARK where PROD_SHIPMARK like @ship_mark + '%'";// + "' and JODOCNO <> 'SMSO124608'";
	...
	//			        	btn_cancel.Location = new Point(558, 912);	
							...
							shipmark_ori_length = Convert.ToInt32(rd["TrxTotalLength"]);
		...
		void retrieve()
		{
			...
			try 
			{
				...
				if(rd_SP2.HasRows)
				{
					while (rd_SP2.Read())
					{				        	
						txtbx_bal_m.Text = rd_SP2["PROD_REJECT_SHRINKAGE"].ToString();
...
//		
		...
		{
			txtbx_total_meter.Text = ((Int32.Parse(txtbx_no_logroll.Text) * Int32.Parse(txtbx_length_cust.Text)) - Int32.Parse(txtbx_bal_m.Text)).ToString();
		}
		...
		{
			if(String.IsNullOrWhiteSpace(txtbx_no_logroll.Text) || String.IsNullOrWhiteSpace(txtbx_length.Text))
				return;
...
//				        'True for "Landscape"
		...
		{
			if(!String.IsNullOrWhiteSpace(txtbx_length.Text) && !String.IsNullOrWhiteSpace(txtbx_total_m.Text) && !String.IsNullOrWhiteSpace(txtbx_bal_m.Text))
				TotalMeterCalculation();
			else
				return;
