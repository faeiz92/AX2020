...
namespace AX2020
{
	...
	public partial class frm_rpt_converting_jr_balance : Form
	{
		...
		public frm_rpt_converting_jr_balance()
		{
			InitializeComponent();
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT * FROM TBL_PROD_CONV_JO_SLITTING_SHIPMARK_R1 WHERE PROD_USERDATETIME BETWEEN @from_date AND @to_date and PROD_BALANCE <> 0", conn);
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_JR_BALANCE.rdl";
					DataSet ds = new DataSet();
