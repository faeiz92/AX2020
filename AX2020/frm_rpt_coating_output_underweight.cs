...
namespace AX2020
{
	...
	public partial class frm_rpt_coating_output_underweight : Form
	{
		...
		public frm_rpt_coating_output_underweight()
		{
			InitializeComponent();
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT * FROM VIEW_AX2020_ERP_COATING_OUTPUT_UNDERWEIGHT WHERE TRXDATE BETWEEN @from_date AND @to_date", conn);
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_COAT_OVERWEIGHT.rdl";
					DataSet ds = new DataSet();
