...
namespace AX2020
{
	public partial class frm_prod_stretch_film : Form
	{
		...
		void Frm_prod_stretch_filmLoad(object sender, EventArgs e)
		{
			ClearAllText(this);
			DataGrid();
...
//			}
		...
		void DataGrid()
		{
			...
			try 
			{
				cmd_SP.CommandText = @"SELECT PROD_DATE, PROD_SHIFT, PROD_DOCNO, PROD_STOCKCODE, PROD_STOCKDESC, PROD_QTY, PROD_UOM FROM TBL_PROD_STRETCH_FILM_OUTPUT ORDER BY PROD_REPORTDATE DESC";
...
//            	ClearAllText(this);
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			{
				if (dt_grid_consume.Rows[i].Cells[4].Value != null)
				{
					rowCount = rowCount + 1;
				}
				else
			...
			if (LineNoGenerator())
			{
				if (sqlConnParmOutput("SP_PROD_STRETCH_FILM_OUTPUT_ADD"))
				{
					for(i=0; i<rowCount; i++)
					{
						...
						if(!sqlConnParmConsume("SP_PROD_STRETCH_FILM_CONSUME_ADD"))
							return;
					}
				}
		...
		private bool LineNoGenerator()
		{
			...
			try 
			{
				cmdNextNumber.CommandText = "Select ProdStretchFilmNextNumber from TBL_ADMIN_NEXT_NUMBER";
				...
				rdNextNumber.Read();
				NextNo = Convert.ToInt32(rdNextNumber["ProdStretchFilmNextNumber"]);
			...
			try
			{
				cmdUpdateNextNumber.CommandText = "update TBL_ADMIN_NEXT_NUMBER set ProdStretchFilmNextNumber = ProdStretchFilmNextNumber + 1";
...
//				cmd_data_consume.Parameters["@SP_PRODC_USER"].Value = lbl_date.Text.ToUpper();
		...
		void Btn_delClick(object sender, EventArgs e)
		{
			...
			if (del == DialogResult.Yes)
            {
			    ...
			    {
					if (sqlConnParmOutput("SP_PROD_STRETCH_FILM_OUTPUT_DEL"))
					{
						for(i=0; i< dt_grid_consume2.Rows.Count;i++)
						{
							...
							if(!sqlConnParmConsume("SP_PROD_STRTECH_FILM_CONSUME_DEL"))
								return;
						}
...
//		void Btn_retrieveClick(object sender, EventArgs e)
		void Retrieve()
		{
			...
			try 
			{
				...
				cmd_SP2.CommandText = @"SELECT PRODC_STOCKCODE, PRODC_STOCKDESC, PRODC_QTY, PRODC_UOM FROM TBL_PROD_STRETCH_FILM_CONSUME where SUBSTRING(PRODC_DOCNO,1,3) = @doc_no";
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			if (dt_grid_consume.Rows.Count < 10)
			{
				...
				using(frm_prod_stretch_film_popup frm_popup_strtech_film =  new frm_prod_stretch_film_popup())
				{
					frm_popup_strtech_film.ShowDialog();
					
					string prod_code_add = frm_prod_stretch_film_popup.prod_code;
					string prod_desc_add = frm_prod_stretch_film_popup.prod_desc;
					string prod_uom_add = frm_prod_stretch_film_popup.prod_uom;
