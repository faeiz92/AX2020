...
namespace AX2020
{
	...
	public partial class frm_prod_warehouse_transfer_receive : Form
	{
		...
		public frm_prod_warehouse_transfer_receive()
		{
			InitializeComponent();
			...
			//Data();
			//DropDownList("SELECT WAREHOUSEID FROM TBL_PROD_WAREHOUSE_AX_WAREHOUSE WHERE FLAG1 = 'FROM' and flag2 = 'REC-JR' ORDER BY WAREHOUSEID", cbx_warehouse_from, "WAREHOUSEID");
			DropDownList("SELECT WAREHOUSEID FROM TBL_PROD_WAREHOUSE_AX_WAREHOUSE WHERE FLAG1 = 'TO' and flag2 = 'REC-JR' ORDER BY WAREHOUSEID", cbx_warehouse_to, "WAREHOUSEID");
		...
		void Data()
		{
			...
			try 
			{
				cmd_search.CommandText = "select * from TBL_PROD_WAREHOUSE_AX_TRANSFER_ORDER" +
					" where (PROD_TOWAREHOUSE = 'GFWSM01' OR PROD_TOWAREHOUSE = 'GFWSM02' OR PROD_TOWAREHOUSE = 'GFWSM03') AND FLAG1 ='Y' AND FLAG1 ='N'";
		...
		bool UpdateToAX()
		{
			...
			try
			{
				...
				cmdProcessData1c.Parameters.AddWithValue("@prod_line", txtbx_shipmark.Text.ToUpper());				//VIEW_SFA_AX_DATA_R9
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			else if(String.IsNullOrWhiteSpace(cbx_warehouse_to.Text) || cbx_warehouse_to.Text == "Please Select")
			{
				MessageBox.Show("Please select warehouse");
				cbx_warehouse_to.Focus();
				return;
			}
...
//			cmd_search = null;
			...
			try 
			{
				cmd.CommandText = "SELECT top 1 * FROM [AX2020StagingDB].[dbo].[VIEW_AXERP_QOH_ALL] a left join [AX2020StagingDB].[dbo].[VIEW_AXERP_PDSBATCHATTRIBUTES_FULLINFO] b " +
					...
					" where (a.INVENTBATCHID like @ship_mark + '%' or B.GRADE like @ship_mark + '%') and a.inventlocationid = 'GFWPM01' and a.availphysical > 1";
				cmd.Parameters.AddWithValue("@ship_mark",  txtbx_shipmark.Text.ToUpper());
				//cmd.Parameters.AddWithValue("@whse",  cbx_warehouse_to.Text);
				cmd.Connection = con;
				con.Open();
...
//						jr_barcode 		= rd["SHIPMARK"].ToString();
			...
			//			Code	LotNo	Customer	S.Mark	Barcode Color	Micron	Width	Length
						...
						if(jr_shipmark != txtbx_shipmark.Text.ToUpper())
						{
							dt_grid_add.Rows.Add(jr_stockcode, jr_lot_no, jr_brand, jr_lot_no, jr_lot_no, jr_color, jr_mic, jr_width, jr_length, cbx_warehouse_to.Text);
			
						}
						else						
							dt_grid_add.Rows.Add(jr_stockcode, jr_lot_no, jr_brand, jr_shipmark, jr_barcode, jr_color, jr_mic, jr_width, jr_length, cbx_warehouse_to.Text);
