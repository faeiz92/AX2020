...
namespace AX2020
{
	...
	public partial class frm_prod_converting_output_shipmark_bal_r1 : Form
	{
		public static string sqlconn = "Server=AX-UAT; Password=sbax2020sbg; User ID=sbax2020; Initial Catalog=SBG_AX_2020; Integrated Security=false";
		string jr_shipmark, jr_barcode;
		
		public frm_prod_converting_output_shipmark_bal_r1()
		{
			InitializeComponent();
		...
		private DataSet GetData()
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT TOP 1 * FROM TBL_PROD_CONV_JO_SLITTING_SHIPMARK_BAL order by PROD_USERDATETIME desc", conn);
		...
		void TempTable()
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				SqlCommand cmd  = new SqlCommand("SP_PROD_CONV_JO_SLITTING_SHIPMARK_BAL", conn);
...
//						jr_color = rd["TrxJoColor"].ToString();	
			...
			try
			{
				...
				reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_SHIPMARK_BAL.rdl";
				DataSet ds = new DataSet();
