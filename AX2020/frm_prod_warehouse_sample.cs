...
namespace AX2020
{
	public partial class frm_prod_warehouse_sample : Form
	{
		...
		public frm_prod_warehouse_sample()
		{
			InitializeComponent();
			this.ControlBox = false;
			lbl_username.Text = "User : " + frm_menu_system.userName;
			
			DropDownList("SELECT * FROM TBL_PROD_WAREHOUSE_SAMPLE_PURPOSE_LIST order by PURPOSE", cbx_purpose, "PURPOSE");				
			//DataGrid();
			dt_grid_consume2.Hide();			
			InitializeDataGridOutput();
		}
		
		void Frm_prod_warehouse_sampleLoad(object sender, EventArgs e)
		{
			ClearAllText(this);
			DataGrid();
...
//			Code	LotNo	Customer	S.Mark	Color	Micron	Width	Length
		...
		void DataGrid()
		{
			...
			try 
			{
				cmd_SP.CommandText = @"SELECT PROD_DATE, PROD_SHIFT, PROD_DOCNO, PROD_STOCKCODE, PROD_STOCKDESC, PROD_QTY, PROD_UOM FROM TBL_PROD_GLUE_OUTPUT where PROD_REPORTDATE>= DATEADD(day,-7, getdate()) ORDER BY PROD_REPORTDATE DESC";
...
//                	((ComboBox)c).Items.Clear(); 
		...
		void Txtbx_stock_codeSelectedIndexChanged(object sender, EventArgs e)
		{
			...
			//string sqlStatement = "select STOCKCODE_GLUE_DESC  from TBL_STOCKCODE_GLUE where STOCKCODE_GLUE = '" + txtbx_stock_code.Text.ToString() +"'";
...
//				while(dbr.Read())
//				{
				//txtbx_desc.Text = (string) dbr["STOCKCODE_GLUE_DESC"].ToString();
//				}
...
//		{
//			string sqlStatement = "select STOCKCODE_GLUE  from TBL_STOCKCODE_GLUE";
...
//			foreach(DataRow dr1 in ds.Tables[0].Rows)
//			{
//				txtbx_stock_code.Items.Add(dr1["STOCKCODE_GLUE"].ToString());
//			}
//				
//		}
...
//		}
		...
		private bool Validation()
        {
               ...
               if (CommonValidation.ValidateIsEmptyString(cbx_purpose.Text.Trim()) || cbx_purpose.Text == "Please Select")
               {
                    DialogBox.ShowWarningMessage(lbl_purpose.Text + " cannot be empty.");
                   	cbx_purpose.Focus();
                   	
                    return false;     
               }
               
               if (CommonValidation.ValidateIsEmptyString(txtbx_requested_by.Text.Trim()))
               {
                    DialogBox.ShowWarningMessage(lbl_requested_by.Text + " cannot be empty.");
                   	cbx_purpose.Focus();
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			if (LineNoGenerator())
			{
				if (sqlConnParmOutput("SP_PROD_GLUE_OUTPUT_ADD"))
				{
					for(i=0; i<dt_grid_consume.RowCount; i++)
					{
						...
						if(!sqlConnParmConsume("SP_PROD_GLUE_CONSUME_ADD"))
							return;
					}
				}
		...
		private bool LineNoGenerator()
		{
			...
			try 
			{
				cmdNextNumber.CommandText = "Select ProdGlueNextNumber from TBL_ADMIN_NEXT_NUMBER";
				...
				rdNextNumber.Read();
				NextNo = Convert.ToInt32(rdNextNumber["ProdGlueNextNumber"]);
...
//				cmd_data.Parameters["@SP_PROD_STOCKDESC"].Value = txtbx_desc.SelectedItem.ToString();
//					
...
//				cmd_data_consume.Parameters["@SP_PRODC_REMARK2"].Value = txtbx_desc.SelectedItem.ToString();
		...
		void Btn_delClick(object sender, EventArgs e)
		{
			...
			if (del == DialogResult.Yes)
            {
			    ...
			    {
					if (sqlConnParmOutput("SP_PROD_GLUE_OUTPUT_DEL"))
					{
						for(i=0; i< dt_grid_consume2.Rows.Count;i++)
						{
							...
							if(!sqlConnParmConsume("SP_PROD_GLUE_CONSUME_DEL"))
								return;
						}
		...
		void Retrieve()
		{
			...
			try 
			{
				cmd_SP1.CommandText = @"SELECT * FROM TBL_PROD_GLUE_OUTPUT where PROD_DOCNO = @doc_no";
...
//					txtbx_oum.Text = rd_SP1["PROD_UOM"].ToString();
			...
			try 
			{
				...
				cmd_SP2.CommandText = @"SELECT PRODC_STOCKCODE, PRODC_STOCKDESC, PRODC_QTY, PRODC_UOM FROM TBL_PROD_GLUE_CONSUME where SUBSTRING(PRODC_DOCNO,1,3) = @doc_no";
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			if (dt_grid_consume.Rows.Count < 12)
			{
				    
				using(frm_prod_warehouse_sample_popup frm_popup_sample =  new frm_prod_warehouse_sample_popup())
				{
					frm_popup_sample.ShowDialog();
					
					string prod_code_add = frm_prod_warehouse_sample_popup.prod_code;
					string prod_desc_add = frm_prod_warehouse_sample_popup.prod_desc;
					string prod_uom_add = frm_prod_warehouse_sample_popup.prod_uom;
