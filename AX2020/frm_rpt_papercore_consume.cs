...
{
	/// <summary>
	/// Description of frm_rpt_papercore_consume.
	/// </summary>
	public partial class frm_rpt_papercore_consume : Form
	{
		...
		public frm_rpt_papercore_consume()
		{
			//
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT PRODC_DATE, PRODC_SHIFT, PRODC_MACHINE, PRODC_DOCNO, 
											...
											FROM TBL_PROD_PAPERCORE_CONSUME WHERE PRODC_DATE BETWEEN @from_date AND @to_date", conn);
			...
			{
				MessageBox.Show("Error - Papercore Consume \nCannot load DB" + ex.Message + ex.ToString());
				return null;
			}
			finally
			{
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath =  @"..\..\report\PROD_PAPERCORE_CONSUME.rdl";
					DataSet ds = new DataSet();
