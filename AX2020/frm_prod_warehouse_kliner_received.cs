...
namespace AX2020
{
	public partial class frm_prod_warehouse_kliner_received : Form
	{
		...
		public frm_prod_warehouse_kliner_received()
		{
			InitializeComponent();
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			if(clickSearch == true)
			{
				if(sqlConnParm("SP_PROD_WAREHOUSE_KLINER_RECEIVED_ADD") & UpdateNotReceived())
				{
					DialogBox.ShowSaveSuccessDialog();
					DataGrid("SELECT PROD_DOCNO, PROD_DATE, PROD_STOCKCODE, PROD_STOCKDESC, PROD_UOM, PROD_QTY FROM TBL_PROD_WAREHOUSE_KLINER_RECEIVED  where PROD_DATE>= DATEADD(day,-30, getdate())", dt_grid);
					DataGrid("SELECT PROD_DOCNO, PROD_DATE, PROD_STOCKCODE, PROD_STOCKDESC, PROD_UOM, PROD_QTY FROM TBL_PROD_KLINER_OUTPUT where PROD_FLAG1 = 'N' and PROD_DATE>= DATEADD(day,-30, getdate())", dt_grid_not_received);
		
				}
			}
//			else
//			{
//				if(sqlConnParm("SP_PROD_WAREHOUSE_KLINER_RECEIVED_EDIT"))
//				{
//					DialogBox.ShowSaveSuccessDialog();
//					DataGrid();
//				}
//			}
		...
		bool UpdateNotReceived()
		{
			...
			try
			{
				cmdUpdate.CommandText = @"UPDATE TBL_PROD_KLINER_OUTPUT SET PROD_FLAG1 = 'Y' where PROD_DOCNO = @doc_no";
			...
			{
				conUpdate.Close();
				MessageBox.Show("Error - Warehouse Kliner Received Update \nCannot update DB" + ex.Message + ex.ToString());
				return false;
			}
			finally 
			{
		...
		void Btn_searchClick(object sender, EventArgs e)
		{
			...
			try 
			{
				cmd_search.CommandText = @"SELECT * FROM TBL_PROD_KLINER_OUTPUT where PROD_DOCNO = @doc_no";
				...
				{
					MessageBox.Show("Error Warehouse Kliner Search : Cannot find JO!");
					return;
				}
			} 
			catch (Exception ex) 
			{
				MessageBox.Show("Error Warehouse Kliner Search : Cannot load DB!" + ex.Message + ex.ToString());
				return;
			} 
			finally 
			{
		...
		void Retrieve()
		{
			...
			try 
			{
				cmd_SP1.CommandText = @"SELECT * FROM TBL_PROD_WAREHOUSE_KLINER_RECEIVED where PROD_DOCNO = @doc_no";
...
//					txtbx_qty.Text = rd_SP1["PROD_QTY"].ToString();
			...
			{
				MessageBox.Show("Error Warehouse Kliner Stock Received Pack Retrieve: Cannot load DB!" + ex.Message + ex.ToString());
				return;
			} 
			finally 
			{
		...
		void Retrieve2()
		{
			...
			try 
			{
				cmd_SP1.CommandText = @"SELECT * FROM TBL_PROD_KLINER_OUTPUT where PROD_DOCNO = @doc_no";
				cmd_SP1.Parameters.AddWithValue("@doc_no",  txtbx_pack_no.Text);
...
//					txtbx_qty.Text = rd_SP1["PROD_QTY"].ToString();
		...
		bool CheckDuplicate()
		{
			...
			try 
			{
				cmd_check_duplicate.CommandText = @"SELECT * FROM TBL_PROD_WAREHOUSE_KLINER_RECEIVED where PROD_DOCNO = @doc_no";
			...
			{
				MessageBox.Show("Error : Warehouse Kliner Stock Receive Pack\nCannot load Duplicate DB!" + ex.Message + ex.ToString());
				return false;
			} 
			finally 
			{
		...
		bool sqlConnParm(string sqlStatement)
		{
			...
			{	
				con_data.Close();
				MessageBox.Show("Error Warehouse Kliner Stock Received \n" + ex.Message + ex.ToString());			
				return false;
			} 
			finally 
			{
		...
		void Frm_prod_warehouse_kliner_receivedLoad(object sender, EventArgs e)
		{
			DataGrid("SELECT PROD_DOCNO, PROD_DATE, PROD_STOCKCODE, PROD_STOCKDESC, PROD_UOM, PROD_QTY FROM TBL_PROD_WAREHOUSE_KLINER_RECEIVED  where PROD_DATE>= DATEADD(day,-30, getdate())", dt_grid);
			DataGrid("SELECT PROD_DOCNO, PROD_DATE, PROD_STOCKCODE, PROD_STOCKDESC, PROD_UOM, PROD_QTY FROM TBL_PROD_KLINER_OUTPUT where PROD_FLAG1 = 'N' and PROD_DATE>= DATEADD(day,-30, getdate())", dt_grid_not_received);
