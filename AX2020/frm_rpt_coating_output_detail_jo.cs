...
namespace AX2020
{
	...
	/// </summary>
	public partial class frm_rpt_coating_output_detail_jo : Form
	{
		...
		public frm_rpt_coating_output_detail_jo()
		{
			//ort.
			//
			InitializeComponent();
	 ...
	 bool sqlConnParm(string cmd_update)
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				cmd.Parameters.Add(new SqlParameter("@SP_DATETIME_START", SqlDbType.DateTime));
				cmd.Parameters["@SP_DATETIME_START"].Value = date_fr;
					
				//cmd.Parameters.Add(new SqlParameter("@SP_DATETO", SqlDbType.NVarChar, 20));
				cmd.Parameters.Add(new SqlParameter("@SP_DATETIME_END", SqlDbType.DateTime));
				cmd.Parameters["@SP_DATETIME_END"].Value = date_to;
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\Production_output_detail_jo2.rdl";
					DataSet ds = new DataSet();
...
//					DataSet ds5 = new DataSet();
					...
					ds = GetData(@"select * from VIEW_COATING_JO");
...
//						reportViewer1.LocalReport.DataSources.Add(rds5);			
		...
		void Btn_generateClick(object sender, EventArgs e)
		{
			...
			if(!sqlConnParm("SP_RPT_PROD_COAT_JO_LIST"))
				return;
