...
namespace AX2020
{
	...
	public partial class frm_prod_converting_jo_r1 : Form
	{
		...
		string username, refNo, QAC_Weight, ctn_bx_code;
		int VSalesQtyOrder, VSalesRctn, VQtyOrder;
        
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        
		public frm_prod_converting_jo_r1()
		{
			InitializeComponent();
		...
		void DropDownListCtnBxCode()
		{
			SqlConnection conn = new SqlConnection(Sqlconn);
            DataTable ds = new DataTable();
            string cmdSql = "select mcode, (mdesc1+ mdesc2) as mdesc from stk where mmc = 'GF' and mstatus = 'A' and mcode like 'ZKCB%' or mmc = 'GF' and mstatus = 'A' and mcode like 'ZK%CB%' order by mdesc";
            SqlDataAdapter sda = new SqlDataAdapter(cmdSql, conn);
...
//		
		...
		void Btn_sbtiClick(object sender, EventArgs e)
		{
			...
			try 
			{
				cmd.CommandText = @"select * from TBL_PROD_CONV_JO where SUBSTRING(JODOCNO, 1, 10) = @doc_no AND SUBSTRING(JODOCNO, (LEN(LEFT(JODOCNO, CHARINDEX ('-', JODOCNO))) + 1), LEN(RIGHT(JODOCNO, CHARINDEX('-', REVERSE(JODOCNO)) - 1))) = @line_no";// + "' and JODOCNO <> 'SMSO124608'";
...
//        	btn_cancel.Location = new Point(566, 1390);
			...
			string ERP_ST_P1_objSqlStatement = "select a.tmc, a.tcode, a.ttype, a.tno, a.tdate, a.tdo_date, a.tdoline1, b.tline, b.tstk, b.tdesc1, b.tdesc2, b.tqty, b.tuom, c.tstring01, c.tstring02, c.tstring03,c.tstring04,c.tdouble01,c.tdouble02 from so a, so_detail b, so_detail_info c where  a.tno = b.tno and a.ttype = b.ttype and a.tmc  = b.tmc and c.tmc  = b.tmc and c.tline = b.tline and c.ttype = b.ttype and c.tno = b.tno and a.tno = '" + txtbx_so_no.Text.ToUpper() + "' and b.tline = '" + txtbx_line_no.Text.ToUpper() + "'";
        	...
        	string ERP_ST_P2_objSqlStatement = "select * from so_detail_pack where  tmc = 'GF' and tno = '" + txtbx_so_no.Text.ToUpper() + "' and tline = '" + txtbx_line_no.Text.ToUpper() + "'";
        	...
        	string ERP_ST_P3_objSqlStatement = "Select * from so_detail_remark where tmc = 'GF' and tno = '" + txtbx_so_no.Text.ToUpper() + "' and tline = '" + txtbx_line_no.Text.ToUpper() + "' order by trem_line";
...
//        	btn_cancel.Location = new Point(631, 1390);
		...
		public bool sqlConnParm2(string sqlStatement)
		{
			...
			try
			{
				...
				cmd_data_add.Parameters["@SP_JOISSUEBY"].Value = frm_menu_system.fullname;
...
//                	((ComboBox)c).Items.Clear();
		...
		{
			int prod_width = Convert.ToInt32(txtbx_width.Text);
			int prod_length = Convert.ToInt32(txtbx_length.Text);
			int jr_length = Convert.ToInt32(new String(cbx_jr_roll.Text.Where(Char.IsDigit).ToArray()));
						
			txtbx_remark0a.Text = Convert.ToString(jr_length/prod_width);
			...
			txtbx_remark0c.Text = Convert.ToString(Convert.ToInt32(txtbx_remark0b.Text)*prod_length);
		...
		private void CalculationPaperCore()
		{
			...
			int pcore_height = Convert.ToInt32(new String(cbx_paper_core.Text.Where(Char.IsDigit).ToArray()));
			txtbx_remark1a.Text = Convert.ToString(pcore_height/prod_width);
