...
namespace AX2020
{
	...
	public partial class frm_prod_converting_output_rewinding_open : Form
	{
		string init = "0";
		...
		public frm_prod_converting_output_rewinding_open()
		{
...
//			EnableControls(btn_report);
		...
		void Frm_prod_converting_output_rewindingLoad(object sender, EventArgs e)
		{
			...
			dtp_date.MinDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 0);
			dtp_date.MaxDate = (new DateTime(DateTime.Now.Year, DateTime.Now.Month, 0));//.AddMonths(1).AddDays(-1);
...
//			}
			...
			if (LineNoGenerator())
			{
				...
				if (sqlConnParm("SP_PROD_CONV_JO_REWINDING_ADD") & sqlConnParmReject("SP_PROD_CONV_JO_REWINDING_REJECT_R2_ADD") & sqlConnParmShipMark("SP_PROD_CONV_JO_REWINDING_SHIPMARK_ADD_R3"))
				{
					...
					for (i = 0; i< Int32.Parse(txtbx_no_logroll.Text); i++)
					{
...
//			{
//				
//				cmd.CommandText = "select top 1 * from VIEW_AXERP_PDSBATCHATTRIBUTES_FULLINFO where INVENTBATCHID = @ship_mark or grade = @ship_mark";
...
//				}
		...
		void Test()
		{
			...
			try 
			{
				...
				cmd.CommandText = "select top 1 * from TBL_PROD_DOUBLE_SIDE_OUTPUT where [PROD_DOUBLESIDE_SHIPMARK] = @ship_mark";
				cmd.Parameters.AddWithValue("@ship_mark",  txtbx_shipping_mark.Text.ToUpper()); //INVENTBATCHID like @ship_mark + '%'
				cmd.Connection = con;
				con.Open();
				SqlDataReader rd = cmd.ExecuteReader(System.Data.CommandBehavior.CloseConnection);
	...
	//			        btn_cancel.Location = new Point(558, 912);	
						...
						jr_shipmark = rd["PROD_DOUBLESIDE_SHIPMARK"].ToString();
						jr_lot_no = rd["PROD_DOUBLESIDE_SHIPMARK"].ToString();
						jr_ref_no = rd["PROD_DOCNO"].ToString();
						jr_micron = "0";
						jr_color = "-";
						jr_barcode = rd["PROD_DOUBLESIDE_SHIPMARK"].ToString();
						jr_stockcode = rd["PROD_STOCKCODE"].ToString();
							
						jr_width = rd["PROD_WIDTH"].ToString();
						shipmark_ori_length = Convert.ToInt32(rd["PROD_LENGTH"]);
						jr_length = rd["PROD_LENGTH"].ToString();
		...
		bool CheckQty()
		{
			...
			try 
			{
				...
				cmd.CommandText = "select TOP 1 * from VIEW_AXERP_QOH_ALL where inventbatchid = @ship_mark";
...
//		
		...
		{
			txtbx_total_meter.Text = (((Int32.Parse(txtbx_no_logroll.Text) + Int32.Parse(txtbx_no_logroll_store.Text)) * Int32.Parse(txtbx_length_cust.Text)).ToString());
		}
