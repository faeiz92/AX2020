...
namespace AX2020
{
	...
	public partial class frm_rpt_glue_consume_stockcode : Form
	{
		...
		public frm_rpt_glue_consume_stockcode()
		{	
			InitializeComponent();
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT * FROM TBL_PROD_GLUE_CONSUME where PRODC_DATE between @from_date and @to_date and (PRODC_MACHINE = (@prodc_machine) or @prodc_machine is null) and (PRODC_STOCKCODE = (@prodc_stockcode) or @prodc_stockcode is null) and (PRODC_REMARK1 = (@prodc_prod_code) or @prodc_prod_code is null) and (PRODC_REMARK2 = (@prodc_prod_desc) or @prodc_prod_desc is null)", conn);
...
//		}
		...
		void Btn_searchClick(object sender, EventArgs e)
		{
			...
			try
			{
				...
				reportViewer1.LocalReport.ReportPath =  @"..\..\report\PROD_GLUE_CONSUME_2.rdl";
				DataSet ds = new DataSet();
