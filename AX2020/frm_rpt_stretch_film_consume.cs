...
namespace AX2020
{
	...
	public partial class frm_rpt_stretch_film_consume : Form
	{
		...
		public frm_rpt_stretch_film_consume()
		{
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT PRODC_DATE, PRODC_SHIFT, PRODC_MACHINE, PRODC_DOCNO, 
											...
											FROM TBL_PROD_STRETCH_FILM_CONSUME WHERE PRODC_DATE BETWEEN @from_date AND @to_date", conn);
			...
			{
				MessageBox.Show("Error - Stretch Film Consume \nCannot load DB" + ex.Message + ex.ToString());
				return null;
			}
			finally
			{
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath =  @"..\..\report\PROD_STRETCH_FILM_CONSUME.rdl";
					DataSet ds = new DataSet();
