...
namespace AX2020
{
	...
	public partial class frm_prod_warehouse_stocktransfer_vm : Form
	{
		...
		public frm_prod_warehouse_stocktransfer_vm()
		{
...
//					" where trxshipmark like @ship_mark +'%' or JRBarcode like @ship_mark +'%' or TrxRawLotNo like @ship_mark +'%'";
...
//			cmd_search = null;
			...
			try 
			{
				cmd.CommandText = "SELECT top 1 * FROM [AX2020StagingDB].[dbo].[VIEW_AXERP_QOH_ALL] a left join [AX2020StagingDB].[dbo].[VIEW_AXERP_PDSBATCHATTRIBUTES_FULLINFO] b " +
					...
					" where a.INVENTBATCHID like @ship_mark and a.inventlocationid = 'GFWVM05' and a.availphysical > 0";
...
//						jr_shipmark 	= "";
			...
			//			Code	LotNo	Customer	S.Mark	Barcode Color	Micron	Width	Length
						...
						if(jr_shipmark != txtbx_shipmark.Text.ToUpper())
						{
							dt_grid_add.Rows.Add(jr_stockcode, jr_lot_no, jr_brand, jr_lot_no, jr_lot_no, jr_color, jr_mic, jr_width, jr_length, "GFWVM05");
...
//                	((ComboBox)c).Items.Clear(); 
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			for(int i=0; i<dt_grid_add.RowCount; i++)
			{
				...
				if(!sqlConnParm("SP_PROD_WAREHOUSE_AX_TRANSFER_ORDER_VM_ADD", i))
					return;
				else
				{
		...
		public bool sqlConnParm(string sqlStatement, int i)
		{
			...
			try
			{
				...
				cmd_data.Parameters["@SP_PROD_LINE"].Value = "W2." + NextNo;
