...
namespace AX2020
{
	public partial class frm_rpt_kliner_output : Form
	{
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT PROD_DATE, PROD_SHIFT, PROD_MACHINE, PROD_DOCNO, 
											...
											FROM TBL_PROD_KLINER_OUTPUT WHERE PROD_DATE BETWEEN @from_date AND @to_date", conn);
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_KLINER_OUTPUT.rdl";
					DataSet ds = new DataSet();
