...
namespace AX2020
{
	...
	public partial class frm_prod_warehouse_stocktransfer : Form
	{
		...
		public frm_prod_warehouse_stocktransfer()
		{
			...
			DropDownList("SELECT WAREHOUSEID FROM TBL_PROD_WAREHOUSE_AX_WAREHOUSE WHERE FLAG1 = 'FROM' ORDER BY WAREHOUSEID", cbx_warehouse_from, "WAREHOUSEID");
			DropDownList("SELECT WAREHOUSEID FROM TBL_PROD_WAREHOUSE_AX_WAREHOUSE WHERE FLAG1 = 'TO' ORDER BY WAREHOUSEID", cbx_warehouse_to, "WAREHOUSEID");
		...
		void Btn_addClick(object sender, EventArgs e)
		{
			...
			try 
			{
				cmd.CommandText = "SELECT top 1 * FROM [AX2020StagingDB].[dbo].[VIEW_AXERP_QOH_ALL] a left join [AX2020StagingDB].[dbo].[VIEW_AXERP_PDSBATCHATTRIBUTES_FULLINFO] b " +
					...
					" where a.INVENTBATCHID like @ship_mark and a.inventlocationid = @whse and a.availphysical > 0";
