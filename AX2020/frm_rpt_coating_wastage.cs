...
namespace AX2020
{
	...
	/// </summary>
	public partial class frm_rpt_coating_wastage : Form
	{
		...
		public frm_rpt_coating_wastage()
		{
			//ort.
			//
			InitializeComponent();
	 ...
	 bool sqlConnParm(string cmd_update)
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath =@"..\..\report\Production_Wastage_LATEST7.rdl";
					DataSet ds = new DataSet();
					...
					ds = GetData(@"select * from VIEW_RPT_TRX_WASTAGE");
					ds2 = GetData(@"select * from VIEW_TOTAL_RAW_MATERIAL_WASTAGE");
					ds3= GetData(@"SELECT * FROM VIEW_TOTAL_RAW_MATERIAL_WASTAGE_MACHINE");
					ds4 = GetData(@"SELECT * FROM VIEW_TOTAL_RAW_MATERIAL_WASTAGE_OPERATORS");
					ds5 = GetData(@"SELECT * FROM VIEW_TOTAL_RAW_MATERIAL_WASTAGE_DETAIL_OUTSPEC_ONLY");
					//ds2 = GetData2();
		...
		void Btn_generateClick(object sender, EventArgs e)
		{
			...
			if(!sqlConnParm("SP_AX2020_RPT_TRX_WASTAGE_LATEST"))
				return;
