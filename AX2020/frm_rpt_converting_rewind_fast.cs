...
namespace AX2020
{
	public partial class frm_rpt_converting_rewind_fast : Form
	{
		...
		public frm_rpt_converting_rewind_fast()
		{
			InitializeComponent();
		...
		private DataSet GetData()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT PROD_MACHINE, SUM(PROD_QTYCTN) AS SUMQTYCTN, SUM(PROD_M2) AS SUMM2, SUM(PROD_TOTALROLL) AS SUMTOTALROLL FROM TBL_PROD_CONV_JO_QUICK_REWIND_SUMMARY_TEMP GROUP BY PROD_MACHINE ORDER BY PROD_MACHINE", conn);
			try
			{
		...
		private DataSet GetData2()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT * FROM TBL_PROD_CONV_JO_QUICK_REWIND_TEMP", conn);
			try
			{
		        SqlDataAdapter dataadapter = new SqlDataAdapter(cmd);
		        conn.Open();
...
//			else
//			{
				sqlConnParm("SP_PROD_CONV_JO_QUICK_REWIND");
//				if(!Report())
//					return;
//			}			
		}
		...
		void sqlConnParm(string cmd_update)
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_REWIND_FAST.rdl";
					DataSet ds = new DataSet();
					DataSet ds2 = new DataSet();
					ds = GetData();
					ds2 = GetData2();
