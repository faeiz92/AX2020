...
namespace AX2020
{
	...
	public partial class frm_rpt_converting_cutting_shift_fast : Form
	{
		...
		public frm_rpt_converting_cutting_shift_fast()
		{
		...
		private DataSet GetData()
		{
			...
			SqlCommand cmd = new SqlCommand("Select  PROD_MACHINE, PROD_SHIFT, SUM(PROD_QTYCTN) AS SUMQTYCTN, SUM(PROD_M2) AS SUMM2, SUM(PROD_TOTALROLL) AS SUMTOTALROLL From TBL_PROD_CONV_JO_QUICK_CUTTING_SUMMARY_SHIFT_TEMP GROUP BY PROD_MACHINE, PROD_SHIFT ORDER BY PROD_MACHINE, PROD_SHIFT", conn);
			try
			{
			...
			{
				MessageBox.Show("Error - Converting Cutting Fast \nCannot load DB" + ex.Message + ex.ToString());
				return null;
			}
			finally
			{
		...
		private DataSet GetData2()
		{
			...
			SqlCommand cmd = new SqlCommand("SELECT PROD_DOCNO, PROD_SHIFT, PROD_STARTDATE, PROD_CUSTOMER, PROD_MACHINE, PROD_CODE, PROD_MICRON, PROD_WIDTH, PROD_LENGTH, PROD_COLOR, PROD_CONVERSION, PROD_QTYORDERED, PROD_TOTALROLL, PROD_QTYCTN, PROD_QTYREJECT, PROD_QTYM2 FROM TBL_PROD_CONV_JO_QUICK_CUTTING_SHIFT_TEMP", conn);
			try
			{
		        SqlDataAdapter dataadapter = new SqlDataAdapter(cmd);
		        conn.Open();
...
//			date_to = dtp_date_to.Value.Date.AddDays(1).AddSeconds(-1).ToString();
			...
			if(!sqlConnParm("SP_PROD_CONV_JO_QUICK_CUTTING_SHIFT"))
				return;
		...
		bool sqlConnParm(string cmd_update)
		{
			...
			{
				conn.Open();
				MessageBox.Show(dtp_date_from.Value.Date.ToString());
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_CUTTING_SHIFT_FAST.rdl";
					DataSet ds = new DataSet();
					DataSet ds2 = new DataSet();
					ds = GetData();
					ds2 = GetData2();
