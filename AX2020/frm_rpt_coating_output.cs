...
namespace AX2020
{
	...
	/// </summary>
	public partial class frm_rpt_coating_output : Form
	{
		...
		private DataSet GetData(string cmd_string)
		{
			...
			cmd.Parameters.AddWithValue("@machine", comboBox1.Text);
		...
		private DataSet GetData2(string cmd_string)
		{
			...
			{
				MessageBox.Show("Error - Coating Prod Concumption \nCannot load DB" + ex.Message + ex.ToString());
				return null;
			}
			finally
			{
	 ...
	 bool sqlConnParm(string cmd_update)
		{
			using (SqlConnection conn = new SqlConnection(sqlconn))
			{
				...
				try
				{
					...
					reportViewer1.LocalReport.ReportPath = @"..\..\report\Production_Output2.rdl";
					DataSet ds = new DataSet();
					...
					ds = GetData2(@"select * from view_ax2020_coating_output_1");
					ds2 = GetData2(@"select * from view_ax2020_coating_output_2");
					ds3= GetData2(@"select * from view_ax2020_coating_output_3");
					ds4 = GetData2(@"select * from view_ax2020_coating_output_4");
					ds5 = GetData2(@"select * from view_ax2020_coating_output_5");
					}
					
					else 
					{
					
					ds = GetData(@"select * from view_ax2020_coating_output_1 where TrxMacID = @machine");
					ds2 = GetData(@"select * from view_ax2020_coating_output_2 where TrxMacID = @machine");
					ds3= GetData(@"select * from view_ax2020_coating_output_3 where TrxMacID = @machine");
					ds4 = GetData(@"select * from view_ax2020_coating_output_4 where TrxMacID = @machine");
					ds5 = GetData(@"select * from view_ax2020_coating_output_5 where TrxMacID = @machine");
					//ds2 = GetData2();
					}
		...
		void Btn_generateClick(object sender, EventArgs e)
		{
			...
			if(!sqlConnParm("SP_AX2020_PROD_COATING_OUTPUT"))
				return;
