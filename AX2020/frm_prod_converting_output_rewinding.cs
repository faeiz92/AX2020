...
namespace AX2020
{
	...
	public partial class frm_prod_converting_output_rewinding : Form
	{
		string init = "0";
		...
		string username, jr_lot_no, jr_ref_no, jr_color, jr_shipmark, jr_barcode, jr_stockcode;
		string ref_no = null, line_no = null;//, prod_line = null;
		int NextNo = 0, NextNoShip = 0, shipmark_bal = 0, shipmark_ori_length = 0, prod_line;
		
		double sumAllReject = 0, total =0, qty_ctn_ordered = 0, qty_roll_ordered =0, bal = 0, balance = 0, consume = 0;
		
		
		public frm_prod_converting_output_rewinding()
		{
			InitializeComponent();
		...
		void DataGrid()
		{
			try
			{
				string sql = "SELECT PROD_DOCNO, PROD_LINE, PROD_DATE, PROD_SHIFT, PROD_CUSTOMER, PROD_TOTALROLL, PROD_QTYREJECT, PROD_CODE, PROD_BRAND, PROD_COLOR, PROD_MICRON, PROD_WIDTH, PROD_LENGTH FROM TBL_PROD_CONV_JO_REWINDING WHERE PROD_DATE>= DATEADD(day,-45, getdate())";
...
//            }
		...
		void Btn_saveClick(object sender, EventArgs e)
		{
			...
			if (LineNoGenerator())
			{
				if (sqlConnParm("SP_PROD_CONV_JO_REWINDING_ADD") & sqlConnParmReject("SP_PROD_CONV_JO_REWINDING_REJECT_ADD_R3"))
				{
						//MessageBox.Show("Data is saved");
						//DialogBox.ShowSaveSuccessDialog();
						
						DataGrid();
						UpdateBal("SP_PROD_CONV_UPDATE_REWINDING");
		...
		private bool LineNoGenerator()
		{
			...
			try 
			{
				cmdNextNumber.CommandText = "Select ProdConvRewindNextNumber from TBL_ADMIN_NEXT_NUMBER";
			...
			try
			{
				cmdUpdateNextNumber.CommandText = "update TBL_ADMIN_NEXT_NUMBER set ProdConvRewindNextNumber = ProdConvRewindNextNumber + 1";
		...
		bool sqlConnParmShipMark(string sqlStatement)
		{
			...
			try
			{
				...
				cmd_data_shipmark.Parameters["@SP_PROD_LINENO"].Value = prod_line;
					
					
				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRWIDTH", SqlDbType.Float));
				cmd_data_shipmark.Parameters["@SP_PROD_JRWIDTH"].Value = Double.Parse(txtbx_width.Text);

				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRLENGTH ", SqlDbType.Float));
				cmd_data_shipmark.Parameters["@SP_PROD_JRLENGTH "].Value = shipmark_ori_length;

				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRMICRON", SqlDbType.Float));
				cmd_data_shipmark.Parameters["@SP_PROD_JRMICRON"].Value = Double.Parse(txtbx_mic.Text);
						
					
					
				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRSTOCKCODE", SqlDbType.NVarChar, 50));
				cmd_data_shipmark.Parameters["@SP_PROD_JRSTOCKCODE"].Value = jr_stockcode;	
					
				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRCOLOR", SqlDbType.NVarChar, 50));
				cmd_data_shipmark.Parameters["@SP_PROD_JRCOLOR"].Value = jr_color;	
					
				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRBRAND", SqlDbType.NVarChar, 50));
				cmd_data_shipmark.Parameters["@SP_PROD_JRBRAND"].Value = txtbx_brand.Text;	
					
				cmd_data_shipmark.Parameters.Add(new SqlParameter("@SP_PROD_JRLOTNO", SqlDbType.NVarChar, 50));
				cmd_data_shipmark.Parameters["@SP_PROD_JRLOTNO"].Value = jr_lot_no;	
...
//			try 
//			{
//				cmd_SP1.CommandText = "select * from TBL_PROD_CONV_JO_REWINDING where JODOCNO = '" + txtbx_ref_no.Text + "'";
//				cmd_SP1.Connection = con_SP1;
//				con_SP1.Open();
//				SqlDataReader rd_SP1 = cmd_SP1.ExecuteReader(System.Data.CommandBehavior.CloseConnection);
//				
...
//		
            ...
            if (del == DialogResult.Yes)
            {
				...
				if (this.dt_grid.SelectedRows.Count > 0)
			    {
	            	...
	            	{
			       		UpdateBal("SP_PROD_CONV_UPDATE_REWINDING_DEL");
...
//				try 
//				{
//					cmd_SP1.CommandText = "SELECT * FROM TBL_PROD_CONV_JO_REWINDING where PROD_DOCNO = '" + txtbx_ref_no.Text + "' and PROD_LINENO = '" + line_no + "'";
//					cmd_SP1.Connection = con_SP1;
//					con_SP1.Open();
...
//            	}
		...
		void Btn_checkClick(object sender, EventArgs e)
		{
			...
			try 
			{
				cmd_Check_Duplicate_JO.CommandText = @"select * from TBL_PROD_CONV_JO_REWINDING_SHIPMARK where PROD_SHIPMARK = @ship_mark";// + "' and JODOCNO <> 'SMSO124608'";
