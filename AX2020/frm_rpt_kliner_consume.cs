...
{
	/// <summary>
	/// Description of frm_rpt_kliner_consume.
	/// </summary>
	public partial class frm_rpt_kliner_consume : Form
	{
		...
		private DataSet GetData(DateTime date_fr, DateTime date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT PRODC_DATE, PRODC_SHIFT, PRODC_MACHINE, PRODC_DOCNO, 
											...
											FROM TBL_PROD_KLINER_CONSUME WHERE PRODC_DATE BETWEEN @from_date AND @to_date", conn);
			...
			try
				{
					...
					reportViewer1.LocalReport.ReportPath =  @"..\..\report\PROD_KLINER_CONSUME.rdl";
					DataSet ds = new DataSet();
