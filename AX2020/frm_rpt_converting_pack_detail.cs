...
namespace AX2020
{
	public partial class frm_rpt_converting_pack_detail : Form
	{
		...
		public frm_rpt_converting_pack_detail()
		{
		...
		private DataSet GetData(string date_fr, string date_to)
		{
			...
			SqlCommand cmd = new SqlCommand(@"SELECT a.PROD_DOCNO, b.JODATE, a.PROD_CUSTOMER, a.PROD_CODE, a.PROD_BRAND, a.PROD_COLOR, a.PROD_MICRON, a.PROD_WIDTH, a.PROD_LENGTH, a.PROD_CONVERSION, a.PROD_QTYCTNORDERED, a.PROD_QTYROLLORDERED, a.PROD_QTYORDERED, a.PROD_MACHINE, a.PROD_DATE, a.PROD_SHIFT, a.PROD_OPERATOR, a.PROD_HELPER, a.PROD_QTYCTN, a.PROD_QTYROLL, a.PROD_TOTALROLL, a.PROD_REMARK1 FROM TBL_PROD_CONV_JO_PACKING a INNER JOIN TBL_PROD_CONV_JO b ON b.JODOCNO = a.PROD_DOCNO WHERE PROD_DATE BETWEEN @from_date and @to_date", conn);
...
//			date_to = dtp_date_to.Value.Date.AddDays(1).AddSeconds(-1).ToString();
			...
			try
			{
				...
				reportViewer1.LocalReport.ReportPath = @"..\..\report\PROD_CONV_PROD_DETAIL_OUTPUT.rdl";
				DataSet ds = new DataSet();
		...
		void Dtp_date_fromValueChanged(object sender, EventArgs e)
		{
			dtp_date_from.Format = DateTimePickerFormat.Custom;

			dtp_date_from.CustomFormat = "MMMM";

			dtp_date_from.ShowUpDown = false;
		}
	}
}
