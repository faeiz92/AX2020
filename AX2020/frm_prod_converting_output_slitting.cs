...
namespace AX2020
{
	public partial class frm_prod_converting_output_slitting : Form
	{
		string init = "0";
		...
		public frm_prod_converting_output_slitting()
		{
			InitializeComponent();
			...
			DropDownList("SELECT * FROM TBL_PROD_CONV_JO_MACHINE_LIST where PART = 'SLITTING' order by MACHINE_NO", cbx_machine, "MACHINE_NO");	
...
//					txtbx_no_logroll.Text = Convert.ToString(pcore_length/prod_length);
			...
			{
				txtbx_shipping_mark.Text = txtbx_prod_code.Text;
				clickCheck = true;
...
//				{
//				
//					if (sqlConnParm("SP_PROD_CONV_JO_SLITTING_ADD") & sqlConnParmReject("SP_PROD_CONV_JO_SLITTING_REJECT_ADD"))
//					{
//						DataGrid();
...
//			{
				...
				if (LineNoGenerator())
				{
					...
					if (sqlConnParm("SP_PROD_CONV_JO_SLITTING_ADD") & sqlConnParmReject("SP_PROD_CONV_JO_SLITTING_REJECT_ADD") & sqlConnParmShipMark("SP_PROD_CONV_JO_SLITTING_SHIPMARK_ADD_R3"))
					{
						DataGrid();
						UpdateBal("SP_PROD_CONV_UPDATE");
...
//			}
		...
		bool LineNoGenerator()
		{
			...
			try 
			{
				cmdNextNumber.CommandText = "Select ProdConvSlitNextNumber, ProdConvSlitShipNextNumber from TBL_ADMIN_NEXT_NUMBER";
			...
			{
				conUpdate3.Close();
				MessageBox.Show("Error - Slitting Next Number Shipmark \nCannot update DB" + ex.Message + ex.ToString());
				return false;
			}
			finally 
			{
		...
		bool sqlConnParm(string sqlStatement)
		{
			...
			try
			{
				...
				cmd_data.Parameters["@SP_PROD_REMARK1"].Value = consume.ToString();
				
				cmd_data.Parameters.Add(new SqlParameter("@SP_PROD_REMARK2", SqlDbType.NVarChar, 50));
				cmd_data.Parameters["@SP_PROD_REMARK2"].Value = balance.ToString();
			...
			try 
			{
				...
				if (rd.HasRows)
				{
					...
					{	
						...
						MessageBox.Show(jr_length.ToString());
...
//			}
		...
		bool AllowSave()
		{
			...
			try 
			{
				...
				{	
					if (rd_SP3.HasRows)
					{
						if(rd_SP3["TOTALROLL"] != DBNull.Value || rd_SP3["QTYORDERED"] != DBNull.Value)
						{
					...
					qty  		= (double) 130 / 100 * qtyOrder;
					
					//MessageBox.Show(qty.ToString() + "" + qtyOrder.ToString());
					
					if((qtyRoll + Convert.ToInt32(txtbx_total_roll.Text)) > qty)
					{
						using(frm_prod_converting_output_slitting_permission frm_slit_pass =  new frm_prod_converting_output_slitting_permission())
						{
							frm_slit_pass.ShowDialog();
					
							bool pass = frm_prod_converting_output_slitting_permission.pass;
		...
		bool CheckQty()
		{
			...
			try 
			{
				...
				cmd.CommandText = "select * from VIEW_AXERP_QOH_PM04 where itemid = @stock_code and inventbatchid = @ship_mark";
		...
		string Batch(string id)
		{
			...
			try
			{
				...
				{
					MessageBox.Show("Error 2 - Slitting Check \nCannot find shipping_mark! -" + id);
					//return "false";
				}
			} 
			catch (Exception ex) 
			{
				MessageBox.Show("Error 3 - Slitting Check \nCannot load DB!" + ex.Message + ex.ToString());
				//return "false";
			} 
			finally 
			{
...
//		}
		...
		{
			if(!String.IsNullOrWhiteSpace(txtbx_extra.Text) && !String.IsNullOrWhiteSpace(txtbx_length.Text))
			{
				txtbx_total_m.Text = ((double)Double.Parse(txtbx_extra.Text) + Double.Parse(txtbx_length.Text)).ToString();
		...
		bool NoLRCheck()
		{
			if(Int32.Parse(txtbx_no_logroll.Text) > ((int)(Double.Parse(txtbx_total_m.Text)/Double.Parse(txtbx_length_cust.Text))))
			{
				return false;
			}
			else
